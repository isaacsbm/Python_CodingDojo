...
class User:
    ...
    def is_valid(user_dict):
        ...
        if not  any(char.isdigit() for char in user_dict["password"]):
            is_valid = False
            flash("Password needs to have at least one number")
        if not any(char.isupper() for char in user_dict["password"]):
            is_valid = False
            flash("Password needs at least one uppercase number")
